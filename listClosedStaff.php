...
if (isset($_GET['cat'])){
  if ($_GET['cat']=='staff.solved'){ //this controls what will be displayed so obviously you can have your own options here
  $query = "SELECT * FROM ticket_STAFF WHERE t_STATUS = 'CLOSED' ORDER BY 't_ID'";
  $result = mysqli_query($conn, $query);
  while ($row = mysqli_fetch_row($result)){
    echo "<p><table border='2'> <tr><td>$row[3]</td></tr> <tr><td>$row[0]</td></tr> <tr><td>$row[1]</tr></td> <td>$row[2]</td></table></p>";
    echo "<td><a href=\"edit.php?id=$row[0]\"><th>View</th></a></td";
  }
} else {
